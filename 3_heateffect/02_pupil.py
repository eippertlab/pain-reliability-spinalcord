...
for subject in range(1, 41):
    ...
    for session in range(1, 3):
        ...
        file_pattern = f'{data_dir}{sub}_{ses}_pupil_epochs_ReliabilityRun.csv'
...
pd.to_pickle(data, f'{out_dir}pupil_ReliabilityRun.pickle')
#%% Load variables    
data = pd.read_pickle(f'{out_dir}pupil_ReliabilityRun.pickle')
ids = data["sub"].unique()
sub_n = len(ids)
...
data_sample_max_avg.to_csv(f'{out_dir}peak_pupil_ReliabilityRun.csv')
#Difference
...
#%% Figure 2. Subjective and peripheral physiological responses, PDR
...
with sns.plotting_context('paper', font_scale = 2): 
    ...
    plt.savefig(f'{out_dir}heateffect_pdr_ReliabilityRun.png', bbox_inches='tight', format="png")
    plt.show()
