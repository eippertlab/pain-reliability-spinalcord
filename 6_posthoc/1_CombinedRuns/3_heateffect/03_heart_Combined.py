...
for session in range (1,3):
    ...
    for subject in range(1, 41):
        ...
        if not os.path.exists(result_dir):
            os.mkdir(result_dir)
        physio_files = glob.glob(f'{data_dir}*physio.tsv')
        physio_files = np.array(sorted(physio_files))
        for physio_file in physio_files:
            run = next((r for r in ["run-01", "run-02", "run-03", "run-04", "run-05"] if r in physio_file), None)
            print(run)
            ...
            #get data
            data = pd.read_csv(physio_file, sep="\t")
...
data_sample_max_avg.to_csv(f'{out_dir}peak_hpr_CombinedRuns.csv')
#Difference
...
#%% Subjective and peripheral physiological responses, HPR, CombinedRuns
...
with sns.plotting_context('paper', font_scale = 2):
    ...
    plt.savefig(f'{out_dir}heateffect_hpr_CombinedRuns.png', bbox_inches='tight', format="png")
    plt.show()
