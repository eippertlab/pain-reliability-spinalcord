...
out_dir = f'{project_dir}derivatives/results/CombinedRuns/reliability/'
physio_dir = f'{project_dir}derivatives/results/CombinedRuns/physio/'

#%% Import data
scr = pd.read_csv(f'{physio_dir}peak_scr_CombinedRuns.csv')
scr = scr.rename(columns={"val_scaled":"maxval"}).drop("Unnamed: 0", axis=1)
pdr = pd.read_csv(f'{physio_dir}peak_pupil_CombinedRuns.csv')
pdr = pdr.rename(columns={"val_scaled":"maxval"}).drop("Unnamed: 0", axis=1)
hpr = pd.read_csv(f'{physio_dir}peak_hpr_CombinedRuns.csv')
...
#import ratings
ratings = pd.read_pickle(f'{physio_dir}rating_CombinedRuns.pickle')

#import copes
stats = pd.read_pickle(f'{out_dir}cope_CombinedRuns.pickle')
copes = stats[stats["stat"]=="cope"]
...
#%% Individual values underlying ICC calculation: DHL C6 across days, CombinedRuns
...
#%% Individual values underlying ICC calculation: Dorsal left dilated quadrant C6 across days, CombinedRuns
...
#%% Individual values underlying ICC calculation: Peak SCR across days, CombinedRuns
...
#%% Individual values underlying ICC calculation: Peak PDR across days, CombinedRuns
...
with sns.plotting_context('paper', font_scale=2.2):
    ...
    ax.set_ylim(0,0.2) #check which limits make sense
    ax.set_xlim(0, 0.2)
    diag_line, = ax.plot(ax.get_xlim(), ax.get_ylim(), ls="--", c=".6")
...
#%%  Individual values underlying ICC calculation: HPR across days, CombinedRuns
...
with sns.plotting_context('paper', font_scale=2.2):
    ...
    ax.set_ylim(0,0.5) #check which limits make sense
    ax.set_xlim(0,0.5)
    diag_line, = ax.plot(ax.get_xlim(), ax.get_ylim(), ls="--", c=".6")
