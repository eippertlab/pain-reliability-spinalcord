...
for subject in {1..41}; do
  for session in {1..2}; do
    ...
    for physio_file in $(find $physio_dir/ -maxdepth 1  -name "*physio.tsv"); do
      ...
      #get the run number of the respective physio input
      if [[ $physio_name =~ run-([0-9]+) ]]; then
      ...
      #2. run pnm of the associated fMRI run
      if [ $runpnm = 1 ]; then
    		cd $data_dir
    		for file in $(find $data_dir/ -maxdepth 1 -name "*$run*bold_denoised_moco_all_refined.nii.gz"); do
