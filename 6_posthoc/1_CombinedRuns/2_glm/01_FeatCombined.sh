...
for subject in {1..40}; do
  for session in {1..2}; do
    ...
    out_dir_fsf=$project_dir/derivatives/sub-${sub}/ses-${ses}/func/glm/CombinedRuns
    mkdir -p $out_dir_fsf
    ...
    cd  $data_dir
    for file in $(find $data_dir/ -maxdepth 1 -name  "*bold_denoised_moco_all_refined.nii.gz"); do
      fname=$(basename "$file" | cut -d. -f1)
      out_dir=$out_dir_fsf/$fname
      #get the run number of the respective file
      ...
      if [ $create_fsf = 1 ]; then
        ...
        #prepare timing file
        for event_file in $(find $base_dir/ -maxdepth 1  -name "*$run*events.tsv"); do
      ...
      if [ $normalize_feat = 1 ]; then
        ...
        sct_apply_transfo -i $glm_dir/cope1.nii.gz \
                          ...
                          -w $warp_dir/warp_moco_all_refined_m2PAM50_t2s.nii.gz \
