...
for subject in range(1, 41):
    ...
    for session in range (1,3):
        ...
        data_dir = f'{project_dir}/derivatives/{sub}/{ses}/func/glm/CombinedRuns/extracts/'
        for roi in rois:
            for stat in ['cope', 'zstat']:
...
pd.to_pickle(data, f'{out_dir}cope_CombinedRuns.pickle')
...
# Concatenate all dataframes at once
data = pd.concat(data, ignore_index=True)
pd.to_pickle(data, f'{out_dir}all_stats_CombinedRuns.pickle')

#%% For spatial specificity, average over both sessions
project_dir = '/data/pt_02306/main/data/pain-reliability-spinalcord/'
data_dir = f'{project_dir}derivatives/results/CombinedRuns/glm/cord/'
out_dir = f'{project_dir}derivatives/results/CombinedRuns/spatial_specificity/'
Path(out_dir).mkdir(parents=True, exist_ok=True)
...
project_dir = '/data/pt_02306/main/data/pain-reliability-spinalcord/'
data_dir = f'{project_dir}derivatives/results/CombinedRuns/glm/dh_left_c6/'
out_dir = f'{project_dir}derivatives/results/CombinedRuns/spatial_specificity/'
Path(out_dir).mkdir(parents=True, exist_ok=True)
data= []
