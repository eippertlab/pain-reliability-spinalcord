...
print(f'p-value ttest rating across days: {np.round(test_ratings["p-unc"].values[0], 2)}')

#SCR
test_scr = pg.pairwise_tests(data=scr, dv="maxval", subject="sub", within="ses", alpha=0.05, parametric=True)
print(f'p-value ttest scr peaks across days: {np.round(test_scr["p-unc"].values[0], 2)}')

#test PDR
test_pdr = pg.pairwise_tests(data=pdr, dv="maxval", subject="sub", within="ses",alpha=0.05, parametric=True)
print(f'p-value ttest pdr across days: {np.round(test_pdr["p-unc"].values[0], 2)}')
#test HPR
test_hpr = pg.pairwise_tests(data=hpr, dv="maxval", subject="sub", within="ses",alpha=0.05, parametric=True)
print(f'p-value ttest hpr across days: {np.round(test_hpr["p-unc"].values[0], 2)}')
...
#%%# Test-retest reliability across both days for subjective ratings, peripheral physiological data and BOLD response amplitudes, only CombinedRuns
...
with sns.plotting_context('paper', font_scale = 1.5):
    ...
    plt.savefig(out_dir+'icc_CombinedRuns.png', bbox_inches='tight', format="png", dpi=300)
    plt.show()
