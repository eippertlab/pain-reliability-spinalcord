...
for subject in {1..40}; do
  for session in {1..2}; do
    ...
    dhr_dir=$data_dir/DHR
    out_dir_fsf=$project_dir/derivatives/sub-${sub}/ses-${ses}/func/glm/DHR
    mkdir -p $out_dir_fsf
    ...
    for file in $(find $data_dir/ -maxdepth 1 -name  "*te40ReliabilityRun*bold_denoised_moco_refined.nii.gz"); do
      ...
      out_dir=$out_dir_fsf/${fname}

      if [ $create_fsf = 1 ]; then
        echo "preparing timing file"
        #prepare timing file
        ...
        sed -i "s+/data/pt_02306/main/data/derivatives/preprocessing/sub-01/ses-01/func/therm_denoise_te-40/pnm/sub-01_ses-01_run-01_task-heat-patch-04_acq-te-40_bold_denoised_moco_refined/pnm_evlist_design.txt+${data_dir}/pnm/${fname}/pnm_evlist_dhr.txt+g" $out_dir_fsf/${fname}.fsf
        #outlier file
        ...
        #evlist
        echo "making ev list"
        ev_file=/data/pt_02306/main/code/github/pain-reliability-spinalcord/helper_functions/pnm_evlist_dhr.txt
        sed "s+/data/pt_02306/main/data/pain-reliability-spinalcord/derivatives/sub-01/ses-01/func/preprocessing/pnm/sub-01_ses-01_task-heat_acq-te40ReliabilityRun_run-01_bold_denoised_moco_refined+${data_dir}/pnm/${fname}+g" $ev_file > $data_dir/pnm/${fname}/pnm_evlist_dhr.txt
        sed -i "s+/data/pt_02306/main/data/pain-reliability-spinalcord/derivatives/sub-01/ses-01/func/preprocessing/sub-01_ses-01_task-heat_acq-te40ReliabilityRun_run-01_bold_denoised_moco_refined_params+${data_dir}/${fname}_params+g" $data_dir/pnm/${fname}/pnm_evlist_dhr.txt
      fi
